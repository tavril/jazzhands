 ...
 *       http:/ / www
  ...
  *WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express
  ...
  // in resp
  . The rest of resp can be ignored
  . // function add_dns_ref_record_row( button, dnsrecid, resp )
{
	var offset = 0;
	while ( $( "input#dnsref_DNS_NAME_dnsref_" + dnsrecid + "_new_" + offset )
	  . length )
	  {
		  offset += 1;
	}
	...
	var drop = $( "<select/>",
	  { name : 'dnsref_DNS_TYPE_' + recpart, id : 'dnsref_DNS_TYPE_' + recpart }
	  ) . append( $( "<option/>", { value : 'A', text : 'A' } ),
	  $( "<option/>", { value : 'AAAA',  text : 'AAAA' } ),
	  $( "<option/>", { value : 'CNAME', text : 'CNAME' } ) );

	$( button ) . closest('tr') . before( $( "<tr/>" ) . append(
		  $( "<td>" ) . append(drop),
	  $( "<td>" ) . append( $( "<input/>", { id : 'dnsref_DNS_NAME_' + recpart,
	...
	// // dynamically builds a table
	  for dns records that refer to a given record
	  . // There's perl code in the interface section that probably wants to
	  do this // too, possibly with an edit button,
	  possibly not . XXX // // function build_dns_ref_table( cell, dnsrecid,
		  resp )
	  {
	...
	for ( var i in resp ['records'] ) {
		...
		var r = $('<tr/>', { class: 'dnsroot'} ) . append( $( "<td/>" ) . append(
				$( s)
			),
			$("<td/>") . append( $( '<input/>', { type : 'text',
						...
						value : rec ['dns_name'] } ),
				$( '<select/>', { class : 'irrelevant dnsdomain',
	...
	$( tbl ) . append( $( '<tr/>' ) . append( $( '<td/>',
		  { colspan : 3 } ) . append( $( '<a/>', { href : '#',
			  class : 'dnsaddref' } ) . append( $( '<img/>',
		  { src : '../stabcons/plus.png',
			  alt   : 'Add',
			  title : 'Add',
			  class : 'plusbutton' } ), $( '<input/>', { type : 'hidden',
		  class    : 'dnsrecordid',
		  value    : dnsrecid,
		  disabled : true,
			  } ) ) ) ) );
...
// // makes a dns record editable . This largely prepopulates the drop down
  // selector
  for dns domains // function make_dns_editable( dnsroot, resp )
  {
...
function build_dnsref_table (obj) {
	var id = $( obj ) . find(".dnsrecordid") . attr('value');
	  ...
	  // // removing / adding class there
		until it can be turned to a // spinning wheel . // blt =
		$( obj ) . closest('td') . find( 'div#dnsvalue_' + id );
	  if ( blt . length ) {
			$( blt ) . toggleClass('irrelevant');
	  } else {
			$( obj ) . removeClass('dnsref');
			$. getJSON( '../dns/dns-ajax.pl', url, function(resp) {
					  var div = $( "<div/>", { class : 'dnsvalueref',
						  id : 'dnsvalue_' + id } ) . append(
...
// // sets up events
  for adding dns references to records
  . // function create_dns_reference_jquery(baseobj)
  {
	  // This shows the CNAME / A record reference table . $( baseobj ) . on(
			'click', 'a.dnsref', function(event)
			{
				return build_dnsref_table(this);
			} );

	  // this allows editing of the dns record
		and is largely used to // populate the domain drop down,
		which is prohibitive to prepopulate . // $( baseobj ) . on( 'click',
			'img.intdnsedit', function(event)
			{
				return make_dnslink_editable(event);
			} );

	  // implements the add button on the dns value cell . $( baseobj ) . on(
			'click', 'a.dnsaddref', function(event)
			{
